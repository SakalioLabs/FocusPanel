...
> 0.11.84 把顶部位置把手从“只能上下拖动”升级为跨屏二维吸附：按住把手拖到任意显示器的左半边，Panel 就固定到该屏左侧；拖到右半边则固定到右侧，鼠标所在高度同时决定上、中、下。拖过多屏间隙时选择最近显示器，负坐标和不同 DPI 仍使用物理像素。显示器、边缘和垂直位置通过一次批量更新应用，只重定位一次，不会在三项设置依次变化时来回跳。
