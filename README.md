...
> 0.10.78 起，旧版本闪退可能遗留的桌面图标会在升级后首次启动时自动恢复，不要求用户进入修复工具，也不会弹出阻塞启动的对话框。恢复结果通过非阻塞提示告知；分区元数据继续保留，文件不会移动或删除。以后发生未处理异常或强制结束时，应用仍会根据本地恢复标记自动补做恢复。
...
- `FocusPanel-0.10.78-full.nupkg`：完整更新包。
