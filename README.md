...
> 0.10.96 起，新安装默认始终显示 76px 紧凑栏。用户启动后立即能看到开始、统一搜索、应用任务栏、Focus、状态和时间，不再先猜程序是否运行、再寻找右缘热区；打开的工作区在离开后只折回紧凑栏。首次引导会明确展示这个选择并持久化。旧版本升级继续尊重原设置：明确选择过开或关都不变化，尚无该配置的老用户保持旧自动隐藏行为。
...
> 0.10.91 起，设置页可启用“鼠标离开后保留 76px 紧凑栏”。展开工作区离开后只折回应用任务栏，不再整条消失；常用应用、开始、搜索、Focus、状态和时间保持可见。0.10.96 起新安装默认启用，旧版本升级不强制改变；全屏保护、托盘隐藏与 `Esc` 的明确用户意图仍优先。
...
- `FocusPanel-0.10.96-full.nupkg`：完整更新包。
