...
- 状态中心首屏优先提供 Panel 自有后台与窗口、网络与无线、应用音量、媒体与电池、显示桌面、锁定、电源、主音量和亮度；三组详情互斥展开并显示选中态。快捷设置、通知和输入法归入明确的 Windows 辅助入口。系统操作均返回明确结果；成功后关闭对应 FocusPanel 弹层以免遮挡目标界面，系统拒绝或启动失败时自动回到状态中心显示可操作的替代方式。
