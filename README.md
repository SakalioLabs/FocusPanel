...
> 0.11.29 修复了 0.11.28 全宽图标视图在部分机器上不出现的问题：反向布尔绑定现在会按 WPF 目标属性返回真正的 `Visible/Collapsed`，不会再把布尔值错误塞给可见性属性。开始按钮也不再访问被接管的 `Shell_TrayWnd`，而是直接通过统一输入边界唤起 Windows 开始菜单；右键系统管理菜单保持不变。
...
> 0.11.0 曾让“开始”普通点击直接打开 Windows 开始菜单；0.11.32 已将普通点击升级为 Panel 原生全部应用启动器，Windows 开始菜单降为 `Shift+点击` 和右键菜单中的兼容入口。右键或 `Shift+F10` 仍可打开完整系统管理菜单，虚拟桌面操作也继续位于该菜单。
