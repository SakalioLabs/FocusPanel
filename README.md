...
> 0.10.96 首次让新安装默认始终显示 76px 紧凑栏；0.10.97 将这项可发现性改进扩展到所有“尚未明确保存开关”的升级用户。明确选择过关闭的人仍保持自动隐藏。启动后即可看到开始、统一搜索、应用任务栏、Focus、状态和时间，工作区离开后只折回紧凑栏。
...
> 0.10.92 起，紧凑栏 Focus 入口支持两条更短路径：`Shift+左键` 或 `Shift+Enter` 一步打开上次使用的工作区；右键或 `Shift+F10` 在单层 Fluent 菜单中直接选择桌面收纳、任务、番茄钟、AI 助手或设置。普通左键仍打开 Focus 中心，不增加新的常驻按钮。

> 0.10.91 起，设置页可启用“鼠标离开后保留 76px 紧凑栏”。展开工作区离开后只折回应用任务栏，不再整条消失；常用应用、开始、搜索、Focus、状态和时间保持可见。0.10.97 起缺少明确偏好的新旧安装默认启用，已明确关闭者不变；全屏保护、托盘隐藏与 `Esc` 的明确用户意图仍优先。
...
- 紧凑栏从上到下固定为开始、搜索、统一固定/运行应用列表、Focus 中心、状态中心和时间；五个系统入口顺序稳定，中部应用区独立滚动。普通滚轮始终滚动应用列表，`Ctrl+滚轮`才在鼠标所指的多窗口应用中循环窗口。
...
- 展开工作区直接显示桌面收纳、任务、番茄钟、AI 和设置五个一级入口；紧凑栏 Focus 入口可用 `Shift+左键` 直达上次工作区，或右键在单层菜单中直接选择这五项。普通左键的 Focus 中心继续提供今日概览、最近使用模块和更新提示。状态中心集中音量、静音、网络、电池、通知、输入法、显示桌面和电源操作。
...
Focus 中心顶部提供“今日概览”：以只读方式汇总未完成任务、今日专注和已收纳桌面项目，并显示可以立即推进的任务。概览不会改变业务数据，打开或手动刷新时才读取最新本地快照。
...
- `FocusPanel-0.10.97-full.nupkg`：完整更新包。
