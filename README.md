...
- 多窗口应用无需打开列表即可切换：`Ctrl+左键` 进入下一个窗口并首尾环绕，`Ctrl+滚轮`可双向循环；普通左键仍打开文字窗口列表，普通滚轮仍只滚动应用栏。连续操作会记住刚刚选中的窗口，不等待 WinEvent 快照回写；滚轮使用 90ms 节流抑制高分辨率触控板抖动，单窗口应用不会吞掉应用栏滚动。悬停窗口列表中可用中键或 `Delete` 直接关闭目标窗口，仍只发送正常 `WM_CLOSE`。
...
- 工具提示和读屏名称明确区分“已固定 · 未运行”“后台运行 · 1 个窗口”“已最小化 · 1 个窗口”“正在使用 · 2 个窗口”；辅助操作提示会按启动、还原、切换、最小化和多窗口列表准确变化。
...
- 状态中心首屏固定保留快捷设置、通知、输入法、显示桌面、锁定、电源、主音量和亮度；网络与无线、应用音量、媒体与电池按需展开。系统操作均返回明确结果；成功后关闭 FocusPanel 弹层以免遮挡 Windows 界面，系统拒绝或启动失败时自动回到状态中心显示可操作的替代方式。
