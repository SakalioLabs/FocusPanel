...
- 状态中心首屏优先提供 Panel 自有后台与窗口、网络与无线、应用音量、媒体与电池、输入法、Panel 通知、显示桌面、锁定、电源、主音量和亮度；五组详情互斥展开并显示选中态。Panel 通知保存本次运行中的更新、任务、番茄钟和恢复消息，支持未读、清空与重新执行动作；Windows 系统通知只在详情底部保留明确标注的兼容入口。系统操作均返回明确结果；成功后保持当前状态中心便于连续调整，系统拒绝或启动失败时留在 Panel 显示原因。
