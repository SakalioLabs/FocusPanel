...
> 0.11.95 同时纠正应用抽屉被做成“任务管理器”的错误：窗口跟踪器不再枚举消息窗口、隐藏宿主或无可见窗口的后台进程，只保留 Windows 任务栏语义下的普通、最大化和最小化顶层窗口；用户主动固定但尚未运行的应用仍保留。状态中心入口改为“任务栏应用与窗口”，移除“纯后台”筛选，并再次使用与 76px 紧凑任务栏相同的安全投影，后台宿主无法从其他数据源漏进界面。

![任务栏应用只保留固定项和可切换窗口](docs/images/taskbar-window-source-v095.svg)

> 0.11.95 修复 AI 智能收纳在项目较多时悄悄退化的问题：自动收纳、显式 AI 分类和已收纳项目重新规划三条链路现在都会把全部项目按每批最多 80 个连续规划，不再只处理前 80 个、让后续项目退回扩展名分类。每批独立校验与降级，某一批离线、限流或返回坏 JSON 时不会抹掉前面已经成功的 AI 结果；用户取消仍立即终止。每一批继续只发送受限文件名、扩展名、类型及既有安全语义，不发送完整路径或文件内容。

![AI 智能分区完整处理所有批次并隔离单批失败](docs/images/ai-partition-complete-batches-v095.svg)

> 0.11.94 完善长应用栏的当前窗口定位：当固定项或运行应用被插入、移除、重排后，即使前台应用身份没有改变，Panel 也会把当前应用重新露出在上下滚动按钮之间，不再被新项目挤到不可见区域。只有集合结构真正变化、活动目标变化或提醒目标变化才触发，普通标题与窗口状态刷新不会抢走手动滚动；同一批通知合并为一次，优先显示需要注意的应用，否则显示当前应用。

![任务栏结构变化后自动露出当前或提醒应用](docs/images/taskbar-reveal-after-structure-change-v094.svg)

> 0.11.93 将“后台应用与窗口”彻底移出 76px 紧凑任务栏：默认栏不再常驻后台管理按钮，少占一个 54px 槽位，把空间还给固定应用、普通窗口和最小化窗口。后台抽屉仍保留在状态中心及其右键菜单中，只有用户主动进入时才展开完整数据；从状态中心打开后会自动聚焦搜索框，不会因为入口迁移而降低操作效率。

![后台管理退出紧凑任务栏并迁回状态中心](docs/images/background-drawer-status-only-v093.svg)

> 0.11.92 继续收紧紧凑任务栏的数据口径：刷新提示与运行数量现在只统计确实拥有可切换窗口的应用，不再把后台抽屉观察到的无窗口进程算成“正在运行”；紧凑栏“后台”入口移除全量进程数字角标，避免在默认界面持续暴露任务管理器式杂项。固定应用、普通窗口和最小化窗口保持原有行为，完整后台数据只在用户主动打开独立抽屉后显示。

![紧凑任务栏不再暴露后台进程数量](docs/images/compact-taskbar-clean-surface-v092.svg)

> 0.11.91 纠正紧凑任务栏被做成“迷你任务管理器”的错误：中部应用栏现在只显示用户明确固定的应用，以及确实拥有普通、最大化或最小化窗口的运行应用；未固定且没有任何普通窗口的托盘常驻、消息窗口、后台宿主和无窗口进程全部退出任务栏。最小化窗口仍然保留，用户主动固定的后台应用也尊重固定选择。后台观察数据继续只服务于状态中心的独立“后台与窗口”抽屉，不再污染任务栏、滚动数量或九槽位快捷键。

![紧凑任务栏只保留固定项和有窗口应用](docs/images/compact-taskbar-window-only-v091.svg)

> 0.11.90 补齐窗口专注的持续可见状态：只要仍有本次专注收起的窗口，76px 紧凑栏的应用区顶部就出现“恢复”入口和实时数量角标，单击立即恢复，恢复完成后入口自动消失；完整摘要同时作为工具提示和读屏名称。这个入口不是新的常驻任务栏按钮，普通状态下完全不占空间；专注期间会从应用滚动区内部让出 46px，使紧凑栏总体最低高度保持不变，短屏不会被额外撑高。搜索面板恢复条、右键菜单和 `Ctrl + Alt + F` 继续操作同一会话。

![紧凑栏持续显示窗口专注恢复入口](docs/images/compact-window-focus-restore-v090.svg)

> 0.11.89 把窗口专注从右键菜单提升成真正的一步全局动作：在任意应用中按 `Ctrl + Alt + F`，Panel 不会展开或抢焦点，而是直接保留当前应用全部窗口并收起其他可管理窗口；再按一次只恢复本次收起的窗口。主组合冲突时自动尝试 `Ctrl + Alt + Shift + F`，不占用常见的 `Ctrl + Shift + F`；设置页显示本次会话真实注册结果。当前前台标记短暂滞后或 Panel 自己正持有焦点时，会使用最后一个真实外部窗口定位应用，不按标题猜测。

![从任意应用一键专注并再次按键恢复](docs/images/global-window-focus-hotkey-v089.svg)

> 0.11.88 增加可恢复的“窗口专注”：在统一任务栏右键一个窗口或应用，选择“专注此窗口 / 应用”，Panel 会收起同一桌面上的其他可管理窗口，让当前工作一步进入干净状态。它会记住普通、最大化和原活动窗口，搜索面板顶部持续显示收起数量并提供“恢复窗口”；单个窗口拒绝操作不会中断其余窗口，失败项可再次恢复，正常退出 FocusPanel 也会主动还原。整个过程直接复用 Panel 的窗口跟踪与状态控制，不调用原生任务栏。

![从任务栏右键进入窗口专注并一键恢复](docs/images/window-focus-session-v088.svg)

> 0.11.87 让单个 Panel 管理多块屏幕时不再靠窗口标题猜位置：窗口跟踪快照会记录每个真实窗口所在的显示设备，“全部窗口”在检测到两块以上有窗口的屏幕后自动显示“全部屏幕 / 显示器 N”筛选。搜索结果、状态中心逐窗口列表、应用工具提示和 DWM 实时缩略卡都会直接标出“显示器 1 / 显示器 2 · 主屏”；窗口拖到另一屏后沿用既有位置事件自动更新，显示器断开或所选屏已无窗口时安全回到“全部屏幕”。显示器命名与 0.11.86 的移动菜单复用同一套物理排序，不轮询窗口，也不调用原生任务栏。

![按显示器查看窗口并在实时缩略卡确认所在屏幕](docs/images/window-display-overview-v087.svg)

> 0.11.86 让多屏窗口管理不再受 Panel 所在屏幕限制：在窗口总览、实时缩略卡或任务栏应用的逐窗口菜单中，直接展开“移动到显示器”，即可选择任意已连接显示器；菜单显示主屏、分辨率和物理坐标，当前窗口所在屏幕会勾选且不可重复移动。多窗口应用右键还提供“把此应用的窗口移到显示器”，一次把浏览器、编辑器等同组窗口归拢到指定屏幕，单个窗口拒绝时不会中断其余窗口。

![逐窗口或整应用直接移动到任意显示器](docs/images/direct-window-display-menu-v086.svg)

> 0.11.85 把顶部位置把手从“只能上下拖动”升级为跨屏二维吸附：按住把手拖到任意显示器的左半边，Panel 就固定到该屏左侧；拖到右半边则固定到右侧，鼠标所在高度同时决定上、中、下。拖过多屏间隙时选择最近显示器，负坐标和不同 DPI 仍使用物理像素。显示器、边缘和垂直位置通过一次批量更新应用，只重定位一次，不会在三项设置依次变化时来回跳。

![拖动位置把手跨屏选择左侧或右侧](docs/images/panel-two-dimensional-drag-v084.svg)

> 0.11.83 让任务栏接管健康检查成为状态中心的自动能力：每次打开“状态”都会刷新最近结果，不必再手动进入设置。检查采用单实例合并，同一时刻只运行一次，两个入口同步显示“正在检查…”并暂时禁用；启用、停用、安全恢复或更新前恢复发生后，旧检查结果会因版本和状态不一致而被丢弃，不会把过期的“接管正常”覆盖到已恢复状态上。

![状态中心自动刷新并拒绝过期任务栏结果](docs/images/taskbar-health-single-flight-v083.svg)

> 0.11.82 新增完整的屏幕左侧固定模式：设置页可选择“屏幕左侧 / 屏幕右侧”，再独立选择上、中、下。切到左侧后，76px 紧凑栏、工作区展开方向、12px 物理热区、3px 运行指示条、窗口实时缩略卡和位置菜单会一起镜像；自动屏幕模式也会选择最左侧显示器。边缘与位置保存到现有设置，多屏负坐标和不同 DPI 下仍使用物理坐标定位。

![Panel 在屏幕左侧或右侧完整镜像](docs/images/panel-left-right-edge-v082.svg)

> 0.11.81 修复运行应用在紧凑任务栏中整组消失的问题：按 Windows 任务栏语义保留带 `APPWINDOW` 标记的有所有者窗口，启动后和 Panel 每次重新出现时都会主动刷新；若应用区仍为空，会直接显示“刷新”按钮，不再只留一块无法操作的空白。顶部位置入口也从难辨认的细把手改为明确显示“位置 · 上 / 中 / 下”，单击选择、上下拖动和多屏持久化继续共用同一套定位逻辑。

![运行应用恢复与右上中下固定位置](docs/images/running-apps-visible-anchor-v081.svg)

> 0.11.80 把任务栏接管健康结果放进日常状态中心：打开“状态”即可看到最近检查结果并重新检查，不必进入设置。接管异常或守护进程已安全恢复时，紧凑栏状态按钮右下角出现独立 `!`，不会和右上角 Panel 通知数量混在一起；正常时不常驻打扰。读屏名称也会同时播报任务栏健康摘要。

![状态中心直接显示任务栏健康并在紧凑栏告警](docs/images/taskbar-health-in-status-center-v080.svg)

> 0.11.79 给任务栏替代增加可验证的“一键检查接管状态”：设置页会分别检查 Explorer 宿主、原生边缘呼出、主屏工作区和窗口区域/DWM 抑制层，正常时说明实际保护方式，异常时直接指出是哪一层失效。检查只读并在后台运行，不会自动隐藏、修复、写工作区或重启 Explorer；成功接管后会自动检查一次，也可随时手动复查。

![设置页一键检查任务栏接管四项状态](docs/images/taskbar-readonly-health-check-v079.svg)

> 0.11.78 让可滚动任务栏变得可预期：应用过多时，上下箭头会直接显示“上方还有几个 / 下方还有几个”，滚轮、按钮、键盘、拖拽以及活动应用自动露出后都会实时更新。到达列表边缘时对应箭头消失，数量超过 99 时显示 `99+`；读屏和工具提示也会播报同一信息。

![任务栏滚动箭头显示隐藏应用数量](docs/images/taskbar-overflow-count-v078.svg)

> 0.11.77 把短屏适配从“按钮变小”升级为真正的结构调整：低于 640 DIP 时，冗余的后台入口并入状态中心，收纳与任务合并为 Focus 中心，紧凑栏固定入口从八个降到六个，为运行应用和滚动控制再释放 88 DIP。点击 Focus 会回到最近模块，展开后顶部导航仍能一步进入收纳、任务、番茄钟、AI 和设置；普通屏幕保持完整入口。

![短屏六入口为运行应用保留空间](docs/images/short-screen-six-entry-dock-v077.svg)

> 0.11.76 恢复紧凑栏里的所有已识别运行应用，不再把只有后台宿主的应用直接过滤掉；短屏和高 DPI 下，固定入口会自动收紧到 44px，为中间应用区腾出空间。顶部位置把手现在单击就直接显示“右上 / 右中 / 右下”，当前档位有勾选，多屏选择仍在同一菜单里。

![短屏保留运行应用并直接选择右侧固定位置](docs/images/running-apps-adaptive-anchor-v076.svg)

> 0.11.75 把多屏切换从设置页提到位置把手：右键紧凑栏顶部细把手即可打开“移动到屏幕”，直接选择自动最右、Windows 主屏或某一块具体显示器；当前选择会勾选，菜单每次打开都会刷新。切换继续走原有物理坐标、Per-Monitor DPI、热区和持久化链，具体屏幕断开时临时回退、重新连接后恢复，不新增另一套定位状态。

![从位置把手直接把 Panel 移到任意屏幕](docs/images/direct-display-target-menu-v075.svg)

> 0.11.74 把“显示桌面”从隐藏手势变成紧凑任务栏最末端的独立入口：直接点击“桌面”即可显示桌面，再点一次恢复刚才的窗口，不必展开工作区，也不发送 `Win+D` 或唤出原生任务栏。按钮保持 44px 点击区域并使用统一视觉令牌；时间按钮原有的 Shift/中键兼容操作仍保留。

![紧凑任务栏独立显示与恢复桌面入口](docs/images/direct-desktop-toggle-v074.svg)

> 0.11.73 让多窗口应用的普通点击更接近、也更直接于原生任务栏：点击应用图标会在旁边固定 FocusPanel 自有 DWM 实时缩略卡，不再先展开完整工作区；再次点击同一图标或按 `Esc` 只收起缩略卡。悬停卡可以原地转为固定态，鼠标移出不会消失；窗口太多时可从卡片底部进入完整列表。切换、窗口状态、半屏/四角排列和正常关闭仍在缩略卡内完成。

![多窗口应用点击固定实时缩略卡](docs/images/pinned-live-window-preview-v073.svg)

> 0.11.72 让 Panel 位置调整从设置项变成直接操作：紧凑栏顶部的细把手可单击循环右上、右中、右下，也可沿右缘上下拖动并按目标屏幕三分区吸附。判断使用物理鼠标坐标，适配多屏负坐标和不同 DPI；拖动期间不会自动收起。右缘 3px 指示条也会缩短到 Panel 实际高度并跟随锚点，不再整屏显示一条与位置无关的边。

![拖动位置把手与锚点指示条](docs/images/panel-drag-anchor-indicator-v072.svg)

> 0.11.71 进一步把任务栏和后台抽屉分清：紧凑栏只保留用户固定应用与拥有可见窗口的运行应用，未固定的纯后台宿主不会再占满滚动区，但仍可在 Panel 自有“后台”抽屉中搜索、固定和管理。键盘导航、自动露出、拖拽与九槽位快捷键全部跟随紧凑栏真实顺序。还可在紧凑栏空白处右键，直接把 Panel 固定到右上、右中或右下，当前选择会勾选并保存。

![紧凑任务栏与后台抽屉分层](docs/images/compact-taskbar-background-drawer-v071.svg)

> 0.11.70 修复紧凑任务栏看不到运行应用的问题：Panel 自动收起后仍持续接收 WinEvent 窗口变化，纯后台宿主排在普通可切换窗口之后，应用区在短屏和缩放环境下也保留最低可见高度。设置页同时新增右上、右中、右下三档固定位置，按所选显示器的真实 DPI 和物理坐标即时移动并记忆选择。

![运行应用恢复与右侧三档固定位置](docs/images/panel-right-anchor-running-apps-v070.svg)

> 0.11.69 把全局窗口总览进一步做成连续切换器：Panel 会根据既有 WinEvent 前台事件维护当前会话的真实窗口 MRU 顺序。第一次按 `Ctrl + Alt + W`（或设置页显示的备用组合）直接预选上一个窗口，`Enter` 即可切回；保持总览打开时重复按快捷键会继续向后循环并首尾环绕。窗口关闭或列表变化时安全落到仍存在的候选，不保存标题历史，也不轮询进程。

![按最近使用顺序连续循环 Panel 窗口](docs/images/mru-window-hotkey-cycle-v069.svg)

> 0.11.68 给“全部窗口”增加真正的一步全局入口：从任意应用按 `Ctrl + Alt + W`（冲突时自动改用 `Ctrl + Shift + W`）即可直接打开 Panel 窗口总览。焦点落在窗口列表，方向键选择、`Enter` 切换、`Delete` 正常关闭、`Esc` 返回，全程不发送 `Win+Tab`、不显示原生任务栏，也不重新轮询进程；设置页会显示本次会话实际注册成功的组合。

![全局快捷键一步打开 Panel 窗口总览](docs/images/global-window-overview-hotkey-v068.svg)

> 0.11.67 把“后台应用与窗口”从只能向下翻的长列表升级为可检索抽屉：可直接输入应用名或可靠的 EXE 名称，并按“全部 / 有窗口 / 纯后台”筛选。结果仍来自任务栏唯一的应用集合，固定顺序和运行项稳定顺序都不会被筛选打乱；从紧凑栏“后台”打开后焦点直接进入搜索框，整个过程不调用原生任务栏、隐藏图标区或 Explorer 私有托盘数据。

![Panel 自有应用抽屉支持搜索与窗口状态筛选](docs/images/searchable-background-app-drawer-v067.svg)

> 0.11.66 让实时窗口缩略卡不再只是“看一眼再切换”：每张卡片直接提供“排列”按钮，可把目标窗口放到左/右半屏或任意四角。菜单打开时会锁住预览卡，鼠标移入选项不会触发提前收起；排列仍按窗口所在显示器的真实工作区计算，不发送 `Win+Z`，不依赖被隐藏的任务栏贴靠浮层，也不会先激活 Panel。

![实时窗口缩略卡直接排列半屏与四角](docs/images/live-preview-window-layout-v066.svg)

> 0.11.65 把“任务栏替代”和桌面收纳的三个反复问题做成硬边界：紧凑栏新增可直接点击的“后台”入口，打开的是 Panel 自己的统一应用抽屉，不会再借 Windows“显示隐藏图标”；增量刷新会逐项保留用户选择的 `.ico` 路径与索引；图标模式改用真实宽度实时算列的密排网格，末行左对齐，窗口缩放后也不会退化成一行一个。窗口右键菜单同时加入半屏和四角布局，全程不调用 `Win+Z` 或任务栏贴靠浮层。

![Panel 自有后台入口、自定义 ICO 与自适应密排](docs/images/panel-owned-background-ico-grid-v065.svg)

> 0.11.64 让逐应用音量从“能在 Panel 里调”进一步变成“一句话就能调”：统一搜索直接接受“Chrome 音量 30”“微信静音”“Spotify 取消静音”或 `Spotify volume up 10`。结果严格绑定当前 Core Audio 会话 ID；同名多会话不会被名称猜测合并，音频会话在点击前结束也只会提示并刷新，不会误改主音量或其他应用。搜索空态还会用当前正在播放的应用生成可编辑示例，全程不打开任务栏声音浮层。

![统一搜索直接控制逐应用音量](docs/images/direct-application-audio-command.svg)

> 0.11.63 删除统一搜索里最后一条会跳出 Windows 声音浮层的兼容路径。现在搜索“声音输出、声音设备、输出设备、音频输出、音量混合器”或 `sound output`，只会打开 Panel 自己的“声音与应用音量”，直接调整主音量、静音和每个应用的音量；源码不再发送 `Win+Ctrl+V`，也不存在第二个容易点错的 Shell 结果。Windows 没有公开支持的默认输出端点切换接口，因此不会改用不稳定的私有 COM 接口冒充完整能力。

![声音搜索只进入 Panel 自有音频详情](docs/images/panel-native-audio-search.svg)

> 0.11.62 把原生任务栏很重要的“看缩略图选窗口”补进状态中心：展开任意多窗口应用后，每个窗口行都有明确的实时画面按钮，点击即打开 FocusPanel 自有的无激活 DWM 预览卡，可直接切换、最小化、最大化/还原和关闭。整个过程不显示原生任务栏、不截屏、不轮询窗口像素；DWM 或受保护窗口拒绝预览时会明确退回文字切换。

![状态中心逐窗口打开 DWM 实时缩略图卡](docs/images/status-center-window-live-preview.svg)

> 0.11.61 把这三处实际体验问题一起收口：所谓“隐藏图标”不再有任何原生任务栏调用，状态中心明确提供 Panel 自己的“后台应用与窗口”；快捷方式、网址和文件夹引用的独立 `.ico` 会在收纳前复制进 FocusPanel 图标库，并按完整路径/文件身份绑定；图标视图则改成固定 12px 节奏的密排换行，不再依赖容易算错列宽的虚拟网格。720px 工作区实测一行稳定显示 6 个图标，最后一行从左连续排列，不再出现一行一个或随机大洞。

![Panel 原生后台应用、ICO 固化与密排收纳图标](docs/images/panel-native-background-ico-dense-grid.svg)

> 0.11.60 将“应用与窗口总览”做成状态中心里的 Panel 自有应用抽屉，不再关闭状态中心并跳到搜索浮层。它直接复用紧凑栏唯一的应用身份和排序：固定、运行、后台及仅消息窗口应用都在一处；单窗口直接切换，多窗口原位展开并可逐个切换或关闭，右键仍提供完整应用管理。列表有独立滚动和回收虚拟化，再次点击入口只收起应用抽屉，不会收回整个 Panel，也不调用 Windows 隐藏图标区。

![Panel 状态中心内嵌应用与窗口抽屉](docs/images/panel-inline-application-drawer.svg)
...
> 0.10.95 起，统一搜索曾补齐截图、声音输出、投影和无线显示四个 Windows 兼容入口；其中声音输出的 `Win+Ctrl+V` 路径已在 0.11.63 删除并由 Panel 自有“声音与应用音量”取代。截图、投影和无线显示仍保留为独立系统能力，不依赖原生任务栏窗口句柄。
...
- 全局快速搜索优先使用 `Ctrl+Alt+Space`；若被其他程序占用，自动回退 `Ctrl+Shift+Space`。按下后直接打开统一搜索并全选原查询，不需要再按一次 Enter。全局窗口总览优先使用 `Ctrl+Alt+W`，冲突时回退 `Ctrl+Shift+W`；首次打开直接预选 MRU 上一个窗口，保持打开时重复按快捷键会继续循环。方向键可微调，`Enter` 切换，`Delete` 正常关闭，`Esc` 退出。窗口专注优先使用 `Ctrl+Alt+F`，冲突时回退 `Ctrl+Alt+Shift+F`；按一次收起当前应用之外的窗口，再按一次恢复。设置页会分别显示本次会话实际注册成功的组合；快捷键均被占用时仍可使用紧凑栏与右键菜单入口。
...
- 状态中心内置应用级音量混音器：直接列出默认输出设备上尚未过期的音频会话，显示正在播放状态，并为每个应用提供独立音量与静音，不必再从任务栏打开快捷设置、进入声音混合器。统一搜索也能直接执行“应用名 音量 30”“应用名 静音/取消静音”及英文 `volume up/down` 命令；结果绑定真实会话 ID，同名会话不合并。活动会话优先、名称与会话实例标识稳定，最多显示 12 项并由状态中心整体滚动；系统声音使用明确标签，受保护进程无法读取产品名时降级为进程名，不按窗口标题猜测身份。滑块连续变化按会话合并最后值，不同应用严格串行；写入期间暂停旧快照覆盖，当前会话结束或设备切换时回滚到最后确认值并刷新列表。实现仅使用 Windows 公开的 `IAudioSessionManager2`、`IAudioSessionEnumerator`、`IAudioSessionControl2` 和 `ISimpleAudioVolume`，不读取播放器私有数据，也不影响独占模式音频流。
