...
> 0.11.50 把 FocusPanel 自己的通知从一次性浮窗变成状态中心里的可回看历史：更新可用、任务捕获/完成、番茄钟结束、桌面与任务栏恢复消息都会同时进入“Panel 通知”，显示未读徽标，支持全部已读、清空以及重新执行通知原动作；同一事件按身份更新，不会重复刷屏。Windows 的“读取全部应用通知”公开接口要求带清单能力的安装包身份并由用户单独授权，当前 Velopack 桌面安装版不会伪装成已经拥有此能力；Win+N 只保留在 Panel 通知详情底部并明确标成兼容入口，不再占据状态中心主操作区。
...
- 状态中心首屏优先提供 Panel 自有后台与窗口、网络与无线、应用音量、媒体与电池、输入法、Panel 通知、显示桌面、锁定、电源、主音量和亮度；五组详情互斥展开并显示选中态。Panel 通知跨重启保存最多 50 条更新、任务、番茄钟和恢复消息，紧凑状态入口直接显示未读徽标，`Shift+点击 / Shift+Enter` 一步打开或收起通知详情。Windows 系统通知只在详情底部保留明确标注的兼容入口。系统操作均返回明确结果；成功后保持当前状态中心便于连续调整，系统拒绝或启动失败时留在 Panel 显示原因。
...
遇到异常时，先按紧急恢复快捷键。仍未恢复可重新启动 FocusPanel；启动阶段会检查并恢复遗留会话。程序永远不会结束 Explorer，也不会持续覆盖 Windows 工作区。Panel 不拦截用户主动按下的 Windows 系统快捷键，但自身界面不再发送 Win+A、Win+Space 或 Win+Tab；FocusPanel 自己产生的更新、任务、番茄钟和恢复消息进入 Panel 通知历史。读取其他应用的完整 Windows 通知需要安装包清单能力和用户授权，当前版本只在通知详情底部保留明确标注的 Win+N 兼容入口。第三方托盘图标不会被复制，也不会通过原生任务栏抽屉伪装成 Panel 能力，后台可操作应用统一进入 Panel 自有窗口总览。
