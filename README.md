...
- 全局快速搜索优先使用 `Ctrl+Alt+Space`；若被其他程序占用，自动回退 `Ctrl+Shift+Space`。按下后直接打开统一搜索并全选原查询，不需要再按一次 Enter。全局窗口总览优先使用 `Ctrl+Alt+W`，冲突时回退 `Ctrl+Shift+W`，打开后可用方向键、`Enter`、`Delete` 和 `Esc` 完成窗口选择、切换、关闭与退出。设置页会分别显示本次会话实际注册成功的组合；窗口快捷键均被占用时仍可点击紧凑栏“窗口”。
