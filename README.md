...
> 0.10.98 起，紧凑栏不再用“Focus 中心”充当重复中转层。“工作”按钮普通点击直接进入最近使用的工作区；展开后，概览、桌面收纳、任务、番茄钟、AI 和设置六个入口始终位于标题下方。右键仍可直接选择任意模块。更新状态点保留在工作入口，进入后顶部设置按钮一次可达。
...
- “工作”入口不再切换临时弹层：普通点击始终展开最近工作区，重复点击不会把整个 Panel 收回。搜索、状态中心和时间仍使用互斥弹层状态机，鼠标真正离开后才继续按既有延时自动收起。
...
- 紧凑栏从上到下固定为开始、搜索、统一固定/运行应用列表、工作、状态和时间；五个固定入口顺序稳定，中部应用区独立滚动。普通滚轮始终滚动应用列表，`Ctrl+滚轮`才在鼠标所指的多窗口应用中循环窗口。
...
- 展开工作区直接显示概览、桌面收纳、任务、番茄钟、AI 和设置六个一级入口；紧凑栏“工作”普通点击直达最近工作区，右键在单层菜单中选择全部模块。状态中心继续集中音量、静音、网络、电池、通知、输入法、显示桌面和电源操作。
...
- 电池状态通过单次快照同步读取是否存在、百分比和充电状态；紧凑栏状态入口的一个提示整合网络、音量与电池，不增加额外按钮或破坏五入口布局。
...
“工作”按钮直接展开最近工作区，标题下方固定显示概览、桌面收纳、任务、番茄钟、AI 和设置，不再创建一套重复的 Focus 中心。状态中心只放设备状态、Windows 公开入口与任务栏恢复信息；首屏保留常用动作、主音量和亮度，网络与无线、应用音量、媒体与电池使用三个默认折叠的单层详情区。
